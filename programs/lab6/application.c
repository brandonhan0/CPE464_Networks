...
int createPDU(PDU_* pdu, uint32_t sequenceNum, uint8_t flag, uint8_t* payload, uint16_t payloadLen){
    ...
    */
    ...
    unsigned short crc = in_cksum((unsigned short*)pdu, total_length);
    memcpy(&pdu->checksum, &crc, 2);
    crc = in_cksum((unsigned short*)pdu, total_length);
...
void printPDU(uint8_t* buffer, int pduLength){
    ...
    int crc = in_cksum((unsigned short*)buffer, total_length);
    if(crc == 0){
